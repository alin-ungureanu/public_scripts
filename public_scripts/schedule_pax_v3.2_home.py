...
def threeAM_Worker():
	...
	while(bid != True):
		...
		currentTime = getTime()
		print currentTime
		if (currentTime[_H_] == 0 and currentTime[_M_] == 30 and currentTime[_S_] > 30):
...
def setTourRetourBestPax():
	...
	#for Minerva pyautogui.moveTo(930, 921)
	pyautogui.moveTo(930, 921)
	click(0.3)
	delay(0.3)
	click(0.3)
	delay(2)
...
def getState():
	try:
		response = urllib2.urlopen(
			'https://raw.githubusercontent.com/4linu/general_repo/master/notification')
		html = response.read()
		return html.rstrip()
	except URLError, e:
		print e.reason
	return _STATE_CONTINUE_
...
def schedulePaxLoop():
	...
	while(True):
		...
		if ((currentTime[_M_] == _TARGET_TIME_[_M_] and currentTime[_S_] > _TARGET_TIME_[_S_])):
			state = getState()
			if (state == _STATE_CONTINUE_):
				schedulePax()
				return
			elif (state == _STATE_SHUTDOWN_):
				shutdownComputer()
		#lock screen before somebody from office shows up
		#if (currentTime[_H_] == 7 and currentTime[_M_] > 30):
		#	lockScreen()
